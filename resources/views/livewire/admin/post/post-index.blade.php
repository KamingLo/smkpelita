<div>
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-800 uppercase">Manajemen Postingan</h1>
            <p class="text-gray-500 text-sm font-medium">Daftar berita, pengumuman, dan prestasi SMK Pelita.</p>
        </div>
        ...
        <div class="p-4 border-b border-gray-50 bg-gray-50/50">
            <div class="relative max-w-sm">
                ...
                <input wire:model.live="search" type="text" placeholder="Cari judul..." 
                    class="block w-full pl-10 pr-4 py-2.5 border-gray-200 rounded-xl text-sm focus:ring-blue-600 focus:border-blue-600">
            </div>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left whitespace-nowrap">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-6 py-4 text-xs font-bold text-gray-400 uppercase">Judul Postingan</th>
                        <th class="px-6 py-4 text-xs font-bold text-gray-400 uppercase">Tipe</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-blue-50/30 transition-colors group">
                        ...
                        <td class="px-6 py-4">
                            <span class="px-3 py-1 text-[10px] font-black uppercase tracking-widest rounded-full 
                                {{ $post->type == 'berita' ? 'bg-blue-100 text-blue-700' : 
                                   ($post->type == 'pengumuman' ? 'bg-orange-100 text-orange-700' : 'bg-emerald-100 text-emerald-700') }}">
                                {{ $post->type }}
                            </span>
                        </td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end items-center gap-3">
                                ...
                                <button wire:click="deletePost({{ $post->id }})" wire:confirm="Yakin hapus?" class="text-gray-400 hover:text-red-600 transition-colors">
                    ...
                    @empty
                    <tr><td colspan="3" class="px-6 py-12 text-center text-gray-400">Tidak ada postingan.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>

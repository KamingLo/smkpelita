<div class="max-w-4xl mx-auto">
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-gray-500">Kelola konten berita, pengumuman, atau prestasi sekolah.</p>
        </div>
    ...
    <form wire:submit="save" class="space-y-6">
        <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
            <label class="block text-xs font-bold uppercase tracking-widest text-gray-400 mb-4">Thumbnail Postingan</label>
                ...
                <div class="w-full md:w-64 h-40 rounded-xl bg-gray-50 border-2 border-dashed border-gray-200 overflow-hidden relative group">
                    ...
                    @else
                        <div class="flex flex-col items-center justify-center h-full text-gray-400">
                            ...
                            <span class="text-[10px] font-bold uppercase text-center px-4 italic">Belum ada gambar</span>
                        </div>
                    @endif
                </div>

                <div class="flex-1 space-y-2">
                    <input type="file" wire:model="thumbnail" id="thumbnail" class="hidden" accept="image/*">
                    ...
                    <p class="text-xs text-gray-400">Rekomendasi format <strong>.webp</strong> untuk performa maksimal. Maksimal 2MB.</p>
        ...
        <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-4">
            ...
            <div class="space-y-2">
                <label class="block text-xs font-bold uppercase tracking-widest text-gray-400 mb-2">Slug</label>
                <input wire:model="slug" type="text" placeholder="url-otomatis" 
                    class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm font-medium text-gray-600 focus:ring-2 focus:ring-blue-600 outline-none transition-all">
        ...
        <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
            ...
            >
                ...
                <trix-editor input="trix_input" x-ref="trix" class="trix-content" placeholder="Tuliskan isi di sini..."></trix-editor>
            </div>
            @error('content') <span class="text-red-500 text-xs font-bold mt-2 block">{{ $message }}</span> @enderror
        </div>

        <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-4">
            <h3 class="text-xs font-black uppercase tracking-widest text-blue-600">Optimasi SEO</h3>
            <div class="grid grid-cols-1 gap-4">
                <div>
                    <label class="block text-xs font-bold uppercase tracking-widest text-gray-400 mb-2">Meta Description</label>
                    <textarea wire:model="meta_description" rows="2" placeholder="Ringkasan singkat..."
                ...
                <div>
                    ...
                    <input wire:model="meta_keywords" type="text" placeholder="Contoh: smk pelita, juara lomba" 
                        class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-600 outline-none text-sm">
                </div>
            </div>
        </div>

        <button type="submit" class="w-full py-4 bg-blue-600 text-white font-bold rounded-2xl shadow-lg hover:bg-blue-700 transition-all uppercase tracking-widest text-sm">
            <span wire:loading.remove wire:target="save">{{ $isEdit ? 'Simpan Perubahan' : 'Terbitkan Postingan' }}</span>
